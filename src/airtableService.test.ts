...
describe('AirtableService', () => {
    ...
    describe('API calls', () => {
        ...
        describe('record operations', () => {
            ...
            test('creates records successfully', async () => {
                
                const mockRecord = [ { id: mockRecordId, fields: { name: 'Test' } }, { id: mockRecordId, fields: { name: 'Test1' } } ];
                mockFetch.mockResolvedValueOnce({
                    ok: true,
                    text: () => Promise.resolve(JSON.stringify({ records: mockRecord })),
                });

                const result = await service.createRecords(mockBaseId, mockTableId, [ { fields: { name: 'Test' } }, { fields: { name: 'Test1' } } ]);
